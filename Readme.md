...
```
There is a little version dance going on there that I will come back to later. For the moment assume the version is the hard coded one.
...
The final section is the actual publishing part which pulls it all together.
```
publishing {
    publications {
...
The pom section edits the settings I want into the pom file. Ther version and name etc are done automatically, but I like to add the correct scm, licenses etc in there too.

The final section is repositories in which I specify a local repository actually inside the project. Normally I would specify something like ~/.m2/repository to get to my usual cross-project local repository but this is good for trying things out.

I found the build failed because I had made so many changes to the build.gradle file. I re-imported the project (File->Re-Import Gradle Project) and that problem went away. It was always fine on the command line *outside* the IDE, it failed on the command line inside the IDE though. Anywa a re-import fixed that.
...
Implementation-Vendor: Prometheus Consulting
```
But so far there was nothing published. That is because we don't want to publish on every local build. I can go to the command line and type 'gradle publish' and then when I look inside my build/repository directory (the one I published to) I can see this:
```
.
└── com
    └── madurasoftware
        └── kotlin-library-template
            ├── 0.0.1
            │   ├── kotlin-library-template-0.0.1.jar
            │   ├── kotlin-library-template-0.0.1.jar.md5
            │   ├── kotlin-library-template-0.0.1.jar.sha1
            │   ├── kotlin-library-template-0.0.1-javadoc.jar
            │   ├── kotlin-library-template-0.0.1-javadoc.jar.md5
            │   ├── kotlin-library-template-0.0.1-javadoc.jar.sha1
            │   ├── kotlin-library-template-0.0.1.pom
            │   ├── kotlin-library-template-0.0.1.pom.md5
            │   ├── kotlin-library-template-0.0.1.pom.sha1
            │   ├── kotlin-library-template-0.0.1-sources.jar
            │   ├── kotlin-library-template-0.0.1-sources.jar.md5
            │   └── kotlin-library-template-0.0.1-sources.jar.sha1
            ├── maven-metadata.xml
            ├── maven-metadata.xml.md5
            └── maven-metadata.xml.sha1
```
I have my jar file, and I already know it has the right manifest settings, my javadoc file and my sources file. They are all supported by md5 and sha1 checksums for verification. I can safely publish this to maven central now, though I ought to change its name and put some real code in there first.

Next is to get it publishing to maven central. I need a new block called nexusStaging:
```
nexusStaging {
    packageGroup = "com.madurasoftware" 
}
```
There are some things I need for credentials etc that belong *outside* the build.gradle, specifically in my ~/.gradle/gradle.properties file which looks like this:
```
nexusUsername = ***********************
nexusPassword = ***********************
...
The details on how to do all this are in the [Sonatype instructions](https://central.sonatype.org/pages/producers.html) and they are too long to repeat here. They are one-off things, though. You don't need to do them over and over.

With that in place I just need to type

`gradle publishToNexus`
...
`gradle publishToNexus closeAndReleaseRepository`
...
There is a slight hole in this in that I don't have to update the version number when I release, I only have to tag. So someone might pull the code and look at the version number and be confused. But there are lots of other places I need to edit the version number during a rlease such as the release notes etc. I can cope with one more.
